...
class TestPerturb(unittest.TestCase):
    ...
    def test_perturb_array(self):
        # create two arrays, perturb one. This is to make sure that we get a copy of the input from perturb
        ...
        self.assertLess(np.max(diff2), AMPLITUDE, msg="perturbation is larger than amplitude!")
        self.assertAlmostEqual(mean_diff1, AMPLITUDE * 0.5, msg="perturbation is most likely too small!",
                               delta=AMPLITUDE * 1e-2)
        self.assertAlmostEqual(mean_diff2, AMPLITUDE * 0.5, msg="perturbation did not return a copy of input!",
                               delta=AMPLITUDE * 1e-2)
    ...
    def test_perturb_nc(self):
        ...
        for i, dt in enumerate([np.float32, np.float64]):
            self.data[i] = Dataset("test/dummy{}.nc".format(i), "w")
        ...
        for i in range(2):
            self.data[i] = Dataset("test/dummy{}.nc".format(i), "r")
        ...
        self.assertLess(np.max(diff), AMPLITUDE, msg="perturbation is larger than amplitude!")
        self.assertAlmostEqual(mean_diff, AMPLITUDE * 0.5, msg="perturbation is most likely too small!",
                               delta=AMPLITUDE * 1e-2)

        # to really make sure: create a plot of the difference to be read out manually if you are scared
        fig, ax = plt.subplots(1, 1)
        xx, yy = np.meshgrid(np.linspace(0, 1, ARRAY_DIM), np.linspace(0, 1, ARRAY_DIM))
        cs = ax.contourf(xx, yy, diff)
        fig.colorbar(cs, ax=ax)
        fig.savefig("{}/diff_figure.pdf".format(test_path))
